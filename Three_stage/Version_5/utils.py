...
#         plt.text(j, i, format(cm[i, j], '.2f'), horizontalalignment='center', color='black' if cm[i, j] > thresh else 'black')
...
def plot_confusion_matrix(targets, predictions, classes):
    ...
    # 找出最大值所在位置
    max_pos = np.unravel_index(np.argmax(cm), cm.shape)
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        plt.text(
            ...
            color='white' if (i, j) == max_pos else 'black'
        )
